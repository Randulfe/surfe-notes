...
export const Button = ({
  ...
  return (
    <button
      {...props}
      className={`ring-offset-background focus-visible:ring-ring inline-flex w-full items-center justify-center gap-2 rounded-md whitespace-nowrap transition-colors hover:cursor-pointer focus-visible:ring-0 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 ${getStatusColors({ status, variant })} transition-all duration-200 ${size === "s" ? "p-2 text-sm font-normal" : size === "m" ? "text-md p-4 font-medium" : "p-6 text-lg font-semibold"}`}
